...
import { useSettingContext } from '@context/settings/settingsContext';
import { useState } from 'react';

type PropsType = { openDrawer: any; setOpenDrawer: any };
export function Navigation({ openDrawer, setOpenDrawer }: PropsType) {
  const [profile, setProfile] = useState(false);
  const { setting, setSetting } = useSettingContext();

  const changeColorMode = () => {
    if (setting.mode === 'dark') {
      setSetting({ ...setting, mode: 'light' });
      localStorage.removeItem('darkMode');
    } else {
      setSetting({ ...setting, mode: 'dark' });
      localStorage.setItem('darkMode', 'true');
    }
  };

  const changeRtlMode = () => {
    if (setting.direction === 'ltr') {
      localStorage.setItem('rtl', 'true');
      setSetting({ ...setting, direction: 'rtl' });
    } else {
      localStorage.removeItem('rtl');
      setSetting({ ...setting, direction: 'ltr' });
    }
  };
        ...
        <div className="h-full flex items-center">
          <div className="mr-10 flex items-center">
            ...
            <h3 className="text-base text-gray-800 font-bold tracking-normal leading-tight ml-3 hidden lg:block">
              React Vite And Tailwind Whit Safari
            </h3>
          </div>
        ...
        <div className="visible xl:hidden flex items-center relative">
          <ul className="p-2 w-64 border-r bg-white absolute top-0 -ml-2 rounded right-0 shadow mt-12 lg:mt-16 hidden">
            ...
            <li className="flex xl:hidden  cursor-pointer text-gray-600 text-sm leading-3 tracking-normal mt-2 py-2 hover:text-indigo-700 focus:text-indigo-700 focus:outline-none flex items-center relative">
              ...
              <span className="ml-2">Products_</span>
            </li>
